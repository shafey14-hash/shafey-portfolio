/* ============================================================
   ...
   Loads assets/models/3d_model.optimized.glb and rotates it based
   on the user's scroll progress through the horizontal experience
   (see assets/js/scroll-controller.js, which dispatches the
   `horizontal:progress` event this file listens for).

   This is the ONLY file that touches the 3D model — replace the
   asset, change its size/position, or tune rotation angles all
   from the clearly-marked constants below.
============================================================ */
...
// ============================================================
const MODEL_SCALE = 0.6; // multiply to make the model bigger/smaller
const CAMERA_DISTANCE_FACTOR = 2.5; // higher = camera further away (model looks smaller)
...
// how quickly the model "catches up" to its target rotation each
// frame — lower = smoother/laggier, higher = snappier/more direct
...
function initModelScene() {
  ...
  loader.load(
    ...
    (gltf) => {
      ...
      wrapper.add(model);
      scene.add(wrapper);
      model = wrapper; // rotate the wrapper, not the raw scene node
  ...
  // ============================================================
  // 3. WHERE SCROLL PROGRESS IS CONVERTED INTO ROTATION
  // ============================================================
  const current = { x: 0, y: 0, z: 0 };
  const target = { x: ROTATION_KEYFRAMES[0].x, y: ROTATION_KEYFRAMES[0].y, z: ROTATION_KEYFRAMES[0].z };

  function updateTargetFromProgress(continuous) {
    ...
    const t = clamped - i0;
    const a = ROTATION_KEYFRAMES[i0];
    const b = ROTATION_KEYFRAMES[i1];
  ...
  window.addEventListener("horizontal:progress", (e) => {
    updateTargetFromProgress(e.detail.continuous);
  });
  ...
  function frame() {
    if (modelReady && model) {
      if (reduceMotion) {
        model.rotation.set(target.x, target.y, target.z); // no easing, just snap
      } else {
        current.x = lerp(current.x, target.x, ROTATION_EASE);
        current.y = lerp(current.y, target.y, ROTATION_EASE);
        current.z = lerp(current.z, target.z, ROTATION_EASE);
